...
// ✅ use env in script, not template
...
async function fetchBooks() {
  ...
  try {
    ...
    // supports either { ok,data:{items,total}} or {items,total}
    ...
    <table class="table">
      ...
      <tbody>
        <tr v-for="b in items" :key="b.id">
          ...
          <td class="right">
            <!-- Edit coming later -->
            <button @click="removeBook(b.id)" class="btn danger">Delete</button>
          </td>
        </tr>
...
<style scoped>
/* tiny, clean styles; we’ll Tailwind later */
.input {
  padding: 8px;
